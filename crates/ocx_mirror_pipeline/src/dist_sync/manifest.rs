...
impl ReleaseRow {
    ...
    /// [`verify_digest`](crate::verify::verify_digest).
    ///
    /// # Errors
    ///
...
impl DistManifest {
    ...
    pub fn apply_select(&mut self, select: &ocx_mirror_spec::Select) {
        if let Some(min) = &select.min_version {
            ...
            self.releases
                .retain(|row| ocx_mirror_spec::filter::version_cmp(&row.version, min) != Some(Ordering::Less));
        }
