...
pub const PUSH_TIMEOUT: Duration = Duration::from_secs(3600);
...
/// through [`push_with_retry`] against a fake `ocx`, with no seam to hand a
/// shorter base in, and four real seconds of sleeping on every `task
/// rust:verify` buys nothing that [`push_retry_backoff`]'s own table test does
/// not already pin. The scaling applies only in this crate's own test build, so
/// a test that rides the ladder lives here, not in a crate above.
