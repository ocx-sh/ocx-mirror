...
use ocx_mirror_spec::{AnnounceConfig, WriteTransport};
...
pub const ENV_ANNOUNCE_TOKEN: &str = ocx_config::env::keys::OCX_ANNOUNCE_TOKEN;
...
pub fn announce_credential_present(config: &AnnounceConfig) -> bool {
...
pub fn missing_credential_hint(config: &AnnounceConfig) -> String {
...
fn index_is_gitlab(config: &AnnounceConfig) -> bool {
    ocx_mirror_spec::forge_is_gitlab(&config.index_repo, config.forge_kind())
}
...
// `pub` only because the root crate's announce tests reach it across the
// crate boundary; not part of the documented surface.
#[doc(hidden)]
pub fn build_announce_args(
...
pub const ANNOUNCE_TIMEOUT: Duration = Duration::from_secs(600);
