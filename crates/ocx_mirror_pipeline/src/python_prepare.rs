...
//! not a single bundle.
//!
//! [`orchestrator::prepare_version`]: crate::orchestrator::prepare_version
...
pub async fn prepare_env_version(
    version: &str,
    tasks: &[WheelEnvTask],
    work_dir: &Path,
...
async fn prepare_env_task(
    ...
    //    zip straight into a `tar.zst` layer, so no extracted `content/` ever
    //    exists on disk for a `PT_INTERP` walk to open.
    // 2. Its scan scope would be empty anyway: `resolve_scan_scope` only walks
    //    INTERFACE-visibility `${installPath}`-rooted `Path` vars, and the
    //    composed env's `PATH`/`PYTHONPATH` are private (`EnvBuilder::add_path`
    //    default) — correct, since these serve the env's own launchers.
    // 3. There is no interpreter ELF in this package to check: `python` comes
    //    from the pinned python-build-standalone DEPENDENCY, whose own libc
    //    claim was made when that package was published.
    //
    // Libc correctness is enforced a layer earlier instead, on the input set:
    // the `wheels:` key's `+libc.glibc`/`+libc.musl` os_feature drives
    // `effective_filter`, so a musl leg only ever admits `musllinux`/`any`
    // wheels and a glibc leg only `manylinux`/`any`. Accepted blind spot,
    // shared with the archive path: a `.so` bundled under `lib/site-packages`
    // and loaded via PYTHONPATH is never inspected — those carry no
    // `PT_INTERP`, so even a scan that reached them would pass them.
...
mod tests {
    ...
    /// The `console_pkg` fixture wheel: a copy of the `ocx_python` crate's, kept
    /// under the root's `tests/fixtures/` (adr_bazel_crate_split.md § C2).
    ...
    async fn prepare_env_task_writes_metadata_and_layers() {
        ...
        // Every synthesized launcher dispatches `python`, never `python3`:
        // python-build-standalone ships `python`/`pythonw` on Windows only, so
        // `python3` there resolves to the WindowsApps store-alias stub.
        assert_eq!(
            entrypoints["console-pkg"]["command"], "python",
            "console-script launchers dispatch `python`: {metadata}"
        );
