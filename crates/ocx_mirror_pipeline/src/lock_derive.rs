...
use crate::ocx_cli::{forward_ocx_env, resolve_ocx_binary};
use ocx_mirror_spec::LockOptions;

/// Hang backstop for the one-per-run `ocx package pull` that materializes a
/// non-universal lock's interpreter. Generous for a cold pull of a CPython
/// package; a healthy pull finishes in seconds.
const INTERPRETER_PULL_TIMEOUT: Duration = Duration::from_secs(600);
...
pub async fn materialize_interpreter(interpreter_package: &str) -> Result<PathBuf, String> {
    ...
    // Same hang backstop the push legs get from `push_once`: a wedged pull
    // (stalled registry connection) must not park `pipeline plan` until the
    // job cap. Interpreter packages are tens of MB, so minutes suffice.
    cmd.kill_on_drop(true);
    let output = tokio::time::timeout(INTERPRETER_PULL_TIMEOUT, cmd.output())
        .await
        .map_err(|_| {
            format!(
                "ocx package pull timed out after {}s for '{interpreter_package}'",
                INTERPRETER_PULL_TIMEOUT.as_secs()
            )
        })?
...
pub struct DeriveLockRequest<'a> {
...
pub(crate) struct ResolvedIndex {
    ...
    name: String,
    url: String,
    credential: Option<ocx_mirror_http::auth::Credential>,
}
...
fn resolve_indexes(indexes: &[String]) -> Result<Vec<ResolvedIndex>, String> {
    indexes
        ...
        .map(|url| {
            ...
            let credential = ocx_mirror_http::auth::resolve(&parsed).map_err(|error| error.to_string())?;
            let name = parsed
                .host_str()
...
async fn invoke_uv_compile(request: &DeriveLockRequest<'_>) -> Result<(), String> {
    ...
    // On timeout the future is dropped mid-`wait_with_output`; without this
    // the orphaned `uv` keeps running, holding the output path and CPU.
    cmd.kill_on_drop(true);
...
pub async fn derive_pylock(request: &DeriveLockRequest<'_>) -> Result<Pylock, String> {
...
mod tests {
    ...
    /// [`ocx_mirror_test_support::OCX_ENV_LOCK`]: `plan.rs` and `push.rs` tests
    ...
    async fn env_lock() -> tokio::sync::MutexGuard<'static, ()> {
        ocx_mirror_test_support::OCX_ENV_LOCK.lock().await
    }
