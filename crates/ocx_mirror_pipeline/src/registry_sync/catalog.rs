...
/// source path is a Block-tier defect: `ocx_mirror_source::url_index` is the right
...
async fn validate_index_base_host(
    ...
    let Some(host) = ocx_mirror_spec::registry::index_host(&url) else {
...
fn index_client(
    ...
    let mut builder = ocx_mirror_http::builder()
...
pub async fn validate_root_host(root: &IndexRoot, trusted: &[String], rules: &ProxyRules) -> Result<(), MirrorError> {
    ...
    // normalisation `ocx_mirror_spec::registry::index_host` applies on the index half.
    let host = host
        .strip_prefix('[')
