...
fn expansions_carry_the_source_so_two_sources_collide_rather_than_merge() {
    ...
    let error = ocx_mirror_spec::destination::detect_collisions(&all).expect_err("two sources, one repository");
...
fn one_source_seen_twice_is_a_duplicate_not_a_collision() {
    ...
    ocx_mirror_spec::destination::detect_collisions(&all).expect("the same package twice is not a collision");
}
