...
use crate::ocx_cli::push::{push_exit_is_transient, push_retry_delay};
use crate::ocx_cli::sign::{ResolvedSign, invoke_sign_reference};
use ocx_mirror_error::{MirrorError, sign_exit_code};
...
impl BatchReport {
    ...
    pub fn envelope(&self) -> BatchEnvelope<'_> {
        BatchEnvelope {
...
pub fn plan_targets(published: &[PublishedTag]) -> (Vec<SignTarget>, Vec<SignTarget>) {
...
impl Backfill<'_> {
    ...
    pub async fn run(&self, indexes: &[SignTarget], platforms: &[SignTarget]) -> BatchReport {
