...
use crate::verify::verify_digest;
use ocx_mirror_error::MirrorError;
use ocx_mirror_spec::layout::{LayoutTemplate, RowValues, SnapshotTemplate};
use ocx_mirror_spec::{DistDocs, DistSpec};
...
async fn fetch_manifest(client: &reqwest::Client, spec: &DistSpec) -> Result<DistManifest, MirrorError> {
    ...
    if let Some(credential) = ocx_mirror_http::auth::resolve(&spec.source)? {
    ...
    // the same bytes).
    let bytes = ocx_mirror_http::read_capped(&mut response, spec.source.as_str(), MANIFEST_FETCH_CEILING).await?;
