...
use crate::ocx_cli::push::{PUSH_TIMEOUT, push_once};
use crate::ocx_cli::sign::{ResolvedSign, sign_push_args};
use crate::target_registry;
use ocx_mirror_report::run_summary::LayerReuse;
use ocx_mirror_spec::MirrorSpec;
...
pub async fn enumerate_env_manifests(bundles_dir: &Path) -> Result<Vec<EnvManifest>, String> {
...
/// subprocess.
///
/// `annotations` is the assembled `ocx_mirror_spec::annotations` map, rendered as the
...
pub fn build_env_push_args(
    ...
    args.extend(ocx_mirror_spec::annotations::push_args(annotations));
...
pub async fn invoke_env_push(
    ...
    let ocx_binary = crate::ocx_cli::resolve_ocx_binary()?;

    let report = crate::ocx_cli::push::push_with_retry(
        &ocx_binary,
        &args,
...
pub async fn register_wheel_layers(
    publisher: &Publisher,
    registry: &str,
    platform: &str,
    layers: &[EnvLayer],
...
async fn write_wheel_registration_metadata(layer: &EnvLayer, platform: &str) -> Result<PathBuf, String> {
    ...
    let path = directory.join(format!(
        ...
        layer.wheel_sha256,
        ocx_mirror_spec::platform_key_slug(platform),
    ));
...
mod tests {
    ...
    fn build_env_push_args_orders_flags_and_layers() {
        let _no_ci = ocx_mirror_test_support::no_ci_env();
        let layers = vec![
            env_layer(
    ...
    fn build_env_push_args_carries_the_sign_tail() {
        use ocx_mirror_spec::{KeylessConfig, Ref, SignConfig};
        ...
        let sign = crate::ocx_cli::sign::resolve_sign(&config, &|_| None, &|_| {
    ...
    async fn enumerate_env_manifests_resolves_relative_paths() {
        use crate::python_prepare::{EnvEntry, EnvLayer};
    ...
    async fn enumerate_with_declared_path(temp: &Path, declared: &str) -> Result<Vec<EnvManifest>, String> {
        use crate::python_prepare::EnvEntry;
