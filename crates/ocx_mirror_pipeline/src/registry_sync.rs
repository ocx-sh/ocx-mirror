...
use crate::options::RegistrySyncOptions;
use crate::registry_copy::{self, CopyContext, CopyError, DESCRIPTION_TAG};
use ocx_mirror_error::MirrorError;
use ocx_mirror_spec::{OnError, RegistrySource, RegistrySpec};
...
pub async fn execute_registry_sync(
    ...
    let expansions: Vec<ocx_mirror_spec::destination::Expansion> = sources
        .iter()
        .flat_map(|prepared| plan::expansions(&prepared.plan.as_name, &prepared.plan.work))
        .collect();
    ocx_mirror_spec::destination::detect_collisions(&expansions)?;
...
async fn sync_package(
    ...
    let destination = match &package.destination {
        ...
        PlannedDestination::FromUpstream => {
            let upstream = ocx_mirror_spec::destination::Upstream {
    ...
    if destination.pointer.is_none() {
        ocx_mirror_spec::destination::warn_on_mirror_path_mismatch(
            &package.name,
            &source_registry,
            &source_repository,
...
fn source_read_seam(trusted_hosts: &[String]) -> Index {
    let client = ClientBuilder::new()
        ...
        .extra_roots(ocx_mirror_http::extra_roots().clone())
        .build();
