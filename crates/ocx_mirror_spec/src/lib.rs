...
//! The ocx-mirror spec model: [`MirrorSpec`] (`mirror.yml`), [`RegistrySpec`]
//! (`registry.yml`) and [`DistSpec`] (`dist.yml`) — parsing, `extends:`
//! resolution, the pre-scan, and validation — plus the grammars that
//! validation checks and the pipeline then executes: the version [`filter`]
//! and [`normalizer`], the [`version_platform_map`], [`annotations`], the
//! registry-sync [`glob`] and [`destination`] templates, and the dist
//! [`layout`].
//!
//! The root re-exports this crate as `ocx_mirror::spec`
//! (adr_bazel_crate_split.md § C1, C2); it names no pipeline or command item.
...
// is an internal file boundary, not a new namespace for them to learn — and so
// this module's own `impl MirrorSpec` keeps calling the validators unqualified.
...
pub(crate) use load::*;
// The loaders the root's commands and pipeline call, named so the glob above
// stays crate-internal.
pub use load::{load_dist_spec, load_registry_spec, load_spec, resolve_extends_chain};
...
pub use platform_keys::{
    image_to_container_id, infer_libc_from_image, platform_key_slug, platform_slug, platform_without_features,
};
...
// the root's `lib.rs` (C-008). Its children ride along so the pipeline can name them.
...
pub use validate::{policy_check_notify, validate_test_scripts};
pub use value_source::ValueSource;
pub use variant::{EffectiveVariant, VariantSpec};
pub use verify_config::{DigestPolicy, VerifyConfig};
pub use versions_config::{BackfillOrder, Bound, BoundOrigin, ResolvedBounds, VersionsConfig, resolve_version_bounds};
pub use wheels::{WheelPatterns, base_platform_key, libc_feature};

// `ocx-mirror schema dist` prints the dist types' doc comments verbatim, link
// targets included, and three of them (dist.rs) spell theirs the way the root
// crate did: `crate::spec::…`. The alias keeps those links resolving without
...
pub fn strip_build(version: &Version) -> Version {
