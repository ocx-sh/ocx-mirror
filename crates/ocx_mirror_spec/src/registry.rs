...
//!
//! A different root type from [`MirrorSpec`](crate::MirrorSpec), not a
...
use super::Target;
use crate::destination::DestinationTemplate;
use crate::glob::Glob;
...
///
/// `target` is shared verbatim with [`MirrorSpec`](crate::MirrorSpec) —
...
pub struct RegistrySpec {
    ...
    /// [`mirror_path_mismatch`](crate::destination::mirror_path_mismatch)
    ...
    /// `ocx_package` still classifies as reserved. See `push_canonical_tag` for why
...
impl RegistrySpec {
    ...
    /// [`MirrorSpec::validate`](crate::MirrorSpec::validate) at every
    ...
    ///
    /// Same doctrine as
    /// [`physical_repository`](crate::destination::physical_repository):
...
/// `pipeline::registry_sync::catalog::build_source_index_client` already
/// refuses a *redirect* downgrade; this is the same rule applied to the
...
/// rule's refusal: `validate_index_base_host` (in
/// `ocx_mirror_pipeline::registry_sync::catalog`) refuses it before the first
/// fetch, and reporting one defect twice against two fields helps nobody.
...
///
/// Deliberately **not** [`ConcurrencyConfig`](crate::ConcurrencyConfig):
