...
//! The mirror's error model: [`MirrorError`], its exit-code mapping
//! ([`MirrorError::kind_exit_code`], [`sign_exit_code`], [`tls_exit_code`]),
//! the `From` conversions for the local errors of the extracted crates, and
//! the `source.type: pylock` / `pypi` failure classifiers ([`pylock`],
//! [`pypi`]). Re-exported by the root as `ocx_mirror::error`.
...
pub enum MirrorError {
    ...
    /// body) stays `SourceError` (69). See [`pypi::classify_error`].
    PypiError(String),
...
pub fn sign_exit_code(code: i32) -> ExitCode {
    match code {
...
/// Classify the one error [`ocx_mirror_http::install_extra_roots`] can raise, under
...
// ── Local errors of the extracted crates (adr_bazel_crate_split.md § C3, E2) ──
//
// Each maps back to the variant the code constructed before the split, with
// the same inner message, so `Display` and the exit code are unchanged.

impl From<ocx_mirror_http::HttpError> for MirrorError {
    fn from(error: ocx_mirror_http::HttpError) -> Self {
        use ocx_mirror_http::HttpError;
        match error {
            HttpError::ClientBuild(message) => Self::ExecutionFailed(vec![message]),
            HttpError::Source(message) => Self::SourceError(message),
        }
    }
}

impl From<ocx_mirror_http::auth::AuthError> for MirrorError {
    fn from(error: ocx_mirror_http::auth::AuthError) -> Self {
        match error {
            ocx_mirror_http::auth::AuthError::Usage(message) => Self::SpecUsageError(message),
        }
    }
}

impl From<ocx_mirror_source::github_release::GithubClientError> for MirrorError {
    fn from(error: ocx_mirror_source::github_release::GithubClientError) -> Self {
        match error {
            ocx_mirror_source::github_release::GithubClientError::ClientBuild(message) => {
                Self::ExecutionFailed(vec![message])
            }
        }
    }
}

impl From<ocx_mirror_report::junit::JunitError> for MirrorError {
    fn from(error: ocx_mirror_report::junit::JunitError) -> Self {
        match error {
            ocx_mirror_report::junit::JunitError::Parse(message) => Self::JunitParseError(message),
        }
    }
}

impl From<ocx_mirror_report::discord::WebhookError> for MirrorError {
    fn from(error: ocx_mirror_report::discord::WebhookError) -> Self {
        use ocx_mirror_report::discord::WebhookError;
        match error {
            WebhookError::Unavailable(message) => Self::WebhookUnavailable(message),
            WebhookError::PermissionDenied(message) => Self::WebhookPermissionDenied(message),
        }
    }
}
...
mod tests {
    ...
    /// E2/E3 identity table: every local error of an extracted crate converts
    /// to the variant it replaced, renders the same bytes through it, keeps
    /// its exit code, and is a leaf (no `source()` link would change a
    /// `{err:#}` render in an `anyhow` chain).
    #[test]
    fn local_errors_convert_to_the_variant_they_replaced() {
        use ocx_mirror_http::HttpError;
        use ocx_mirror_http::auth::AuthError;
        use ocx_mirror_report::discord::WebhookError;
        use ocx_mirror_report::junit::JunitError;
        use ocx_mirror_source::github_release::GithubClientError;

        fn check<E>(local: E, expected: ExitCode, is_variant: fn(&MirrorError) -> bool)
        where
            E: std::error::Error + Clone + Into<MirrorError>,
        {
            assert!(local.source().is_none(), "{local:?} must be a leaf");
            let mirror: MirrorError = local.clone().into();
            assert!(is_variant(&mirror), "{local:?} converted to {mirror:?}");
            assert_eq!(mirror.to_string(), local.to_string(), "{local:?}");
            assert_eq!(mirror.kind_exit_code(), expected, "{local:?}");
        }

        check(
            HttpError::ClientBuild("m".into()),
            ExitCode::Failure,
            |e| matches!(e, MirrorError::ExecutionFailed(m) if m == &["m"]),
        );
        check(
            HttpError::Source("m".into()),
            ExitCode::Unavailable,
            |e| matches!(e, MirrorError::SourceError(m) if m == "m"),
        );
        check(
            AuthError::Usage("m".into()),
            ExitCode::UsageError,
            |e| matches!(e, MirrorError::SpecUsageError(m) if m == "m"),
        );
        check(
            GithubClientError::ClientBuild("m".into()),
            ExitCode::Failure,
            |e| matches!(e, MirrorError::ExecutionFailed(m) if m == &["m"]),
        );
        check(
            JunitError::Parse("m".into()),
            ExitCode::DataError,
            |e| matches!(e, MirrorError::JunitParseError(m) if m == "m"),
        );
        check(
            WebhookError::Unavailable("m".into()),
            ExitCode::Unavailable,
            |e| matches!(e, MirrorError::WebhookUnavailable(m) if m == "m"),
        );
        check(
            WebhookError::PermissionDenied("m".into()),
            ExitCode::PermissionDenied,
            |e| matches!(e, MirrorError::WebhookPermissionDenied(m) if m == "m"),
        );
    }
