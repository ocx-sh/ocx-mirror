...
pub fn from_inline(versions: &HashMap<String, UrlIndexVersion>) -> anyhow::Result<Vec<VersionInfo>> {
...
pub async fn from_remote(url: &str) -> anyhow::Result<Vec<VersionInfo>> {
    ...
    let response = ocx_mirror_http::client()?.get(url).send().await?.error_for_status()?;
