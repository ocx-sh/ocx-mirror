...
//! a derived lock, not asset regex matching — see the root's `spec::Source::is_env`).
...
/// # Errors
///
/// The last 404 when no index has the project —
/// `ocx_mirror_error::pypi::classify_error` reads that as malformed input (exit 65). Any
/// transport or parse failure surfaces as itself and classifies as an
/// unavailable source (exit 69).
pub async fn list_versions(package: &str, indexes: &[String]) -> anyhow::Result<Vec<VersionInfo>> {
    let client = ocx_mirror_http::client()?;
    ...
    for index in indexes {
        ...
        if let Some(credential) = ocx_mirror_http::auth::resolve(&url::Url::parse(&url)?)? {
