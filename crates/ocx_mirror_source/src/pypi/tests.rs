...
/// A 500 means "unknown", not "absent": the walk stops with an error rather
/// than falling through to the next index, which would answer here. The
/// rendered message and its exit code are pinned in `ocx_mirror_error`.
#[tokio::test]
async fn a_server_error_stops_the_index_walk() {
    install_crypto_provider();
    ...
    let result = list_versions("pycowsay", &[broken, other]).await;
    broken_server.await.unwrap();
    other_server.abort();

    assert!(result.is_err(), "a 500 must not fall through: {result:?}");
}

/// A body that is neither PEP 691 JSON nor parseable is an error, never an
/// empty version list. Its classification is pinned in `ocx_mirror_error`.
#[tokio::test]
async fn an_unparseable_body_is_an_error() {
    install_crypto_provider();
    let (index, server) = spawn_index(vec![json_ok("{not json")]).await;

    let result = list_versions("pycowsay", &[index]).await;
    server.await.unwrap();
...
async fn attaches_a_configured_credential_to_discovery() {
    install_crypto_provider();
    let guard = ocx_mirror_test_support::OCX_ENV_LOCK.lock().await;
    let (index, server) = spawn_index(vec![json_ok(r#"{"files": []}"#)]).await;
