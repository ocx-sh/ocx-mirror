...
use ocx_mirror_http::retry::jitter;
...
///
/// `ocx_mirror_http::client()` carries the connect bound and nothing else, by
...
/// Why `client` refused to build an octocrab client.
///
/// Carries the message the `MirrorError` it replaced carried and renders
/// byte-identically to it (adr_bazel_crate_split.md § C3, E3).
/// `From<GithubClientError> for MirrorError` maps it back to that variant.
...
/// the same `ocx_mirror_http` factory as every other client the mirror builds.
///
...
/// # Errors
///
/// [`GithubClientError::ClientBuild`] when the TLS backend cannot be
/// built, or when `base` or `token` cannot be put on the wire.
///
/// # Panics
///
/// Outside a Tokio runtime: octocrab buffers the service, and `tower`'s
/// `Buffer` spawns its worker on construction.
pub(crate) fn client(base: &str, token: Option<&str>) -> Result<octocrab::Octocrab, GithubClientError> {
    let failed = GithubClientError::ClientBuild;
    ...
    let transport = MirrorTransport {
        client: ocx_mirror_http::builder()
...
///
/// As `client`.
pub fn api_client(token: Option<&str>) -> Result<octocrab::Octocrab, GithubClientError> {
...
mod tests {
    ...
    // `ocx_mirror_http::builder` takes no input, and `build()` fails only when the
    // TLS backend cannot initialise, which a test cannot provoke.

    /// `client`'s refusal for `base`/`token`, which must never reach octocrab.
    fn client_error(base: &str, token: Option<&str>) -> GithubClientError {
    ...
    /// A format pin only: both sides read this crate's `CARGO_PKG_VERSION`,
    /// so it cannot see the value drift from the binary's. That the two agree
    /// is `tests/workspace_structure.rs` clause (h) (every member inherits the
    /// workspace version).
    #[test]
    fn the_user_agent_format_is_ocx_mirror_slash_version() {
