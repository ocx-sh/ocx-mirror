...
/// What [`resolve`] raises: a half-configured or unknown `OCX_AUTH_<slug>_TYPE`.
///
/// Renders byte-identically to the `MirrorError::SpecUsageError` (exit 64) it
/// replaced (adr_bazel_crate_split.md § C3, E3); `From<AuthError> for
/// MirrorError` maps it back to that variant.
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum AuthError {
    /// `MirrorError::SpecUsageError`.
    #[error("mirror spec usage error: {0}")]
    Usage(String),
}
...
impl Credential {
    ...
    pub fn as_basic_pair(&self) -> (String, String) {
        match self {
...
/// # Errors
///
/// [`AuthError::Usage`] (`MirrorError::SpecUsageError`, exit 64) when
/// `OCX_AUTH_<slug>_TYPE` names a scheme whose variables are missing, or is not
/// one of `anonymous`/`basic`/`token`. An operator who set the variables meant
/// them to be used, so a half-configured identity fails loudly here rather than
...
pub fn resolve(url: &Url) -> Result<Option<Credential>, AuthError> {
...
fn from_env(host: &str) -> Result<Option<Credential>, AuthError> {
    ...
    let auth_type = AuthType::try_from(declared).map_err(|error| AuthError::Usage(format!("{type_env}: {error}")))?;
    let missing = |name: &str| AuthError::Usage(format!("{type_env} is '{auth_type}' but {name} is unset"));
