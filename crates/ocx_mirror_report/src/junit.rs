...
/// What [`parse_str`] and [`parse_async`] raise.
///
/// Renders byte-identically to `MirrorError::JunitParseError` (exit 65), the
/// variant it replaced; `From<JunitError> for MirrorError` maps it back.
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum JunitError {
    /// Malformed XML or an unreadable file.
    #[error("JUNIT parse error: {0}")]
    Parse(String),
}
...
///
/// Returns `Err(JunitError::Parse)` on malformed XML.
pub fn parse_str(xml: &str) -> Result<JunitTestsuite, JunitError> {
    ...
    let report = Report::deserialize_from_str(&xml).map_err(|e| JunitError::Parse(format!("XML parse error: {e}")))?;
...
pub async fn parse_async(path: &Path) -> Result<JunitTestsuite, JunitError> {
    let xml = tokio::fs::read_to_string(path)
        .await
        .map_err(|e| JunitError::Parse(format!("failed to read {}: {e}", path.display())))?;
    parse_str(&xml)
}
...
mod tests {
    ...
    fn parse(path: &std::path::Path) -> Result<JunitTestsuite, JunitError> {
...
<testsuite name="ocx-mirror.shfmt.linux_amd64.ubuntu_2404"
  ...
  <testcase name="version" classname="ocx-mirror.shfmt.linux_amd64.ubuntu_2404" time="1.0"/>
    ...
    // ── E4 characterization: exact messages ────────────────────────────────
    //
    // Pinned byte for byte before the crate split moved this module. The exit
    // code is pinned once, by the `From<JunitError> for MirrorError` identity
    // table in `ocx_mirror_error`'s tests.
