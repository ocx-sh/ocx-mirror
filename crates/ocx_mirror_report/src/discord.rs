...
/// What [`post`] raises.
///
/// Each variant renders byte-identically to the `MirrorError` variant it
/// replaced; `From<WebhookError> for MirrorError` maps each back to it.
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum WebhookError {
    /// `MirrorError::WebhookUnavailable` (exit 69).
    #[error("webhook unavailable: {0}")]
    Unavailable(String),
    /// `MirrorError::WebhookPermissionDenied` (exit 77).
    #[error("webhook permission denied: {0}")]
...
/// # Errors
///
/// - [`WebhookError::Unavailable`] on 5xx, network timeout, or 429
///   after all retries are exhausted.
/// - [`WebhookError::PermissionDenied`] on 401/403.
pub async fn post(webhook_url: &str, payload: &DiscordWebhookPayload) -> Result<(), WebhookError> {
    ...
    // Layered on `ocx_mirror_http::builder`, not `reqwest::Client::builder`: the
    ...
    let client = ocx_mirror_http::builder()
        ...
        .build()
        .map_err(|e| WebhookError::Unavailable(format!("failed to build HTTP client: {e}")))?;
    ...
    loop {
        let response = client.post(webhook_url).json(payload).send().await.map_err(|e| {
            ...
            if e.is_timeout() {
                WebhookError::Unavailable(format!("request timed out: {}", e.without_url()))
            } else {
                WebhookError::Unavailable(format!("network error: {}", e.without_url()))
            }
        })?;
        ...
        if code == 401 || code == 403 {
            return Err(WebhookError::PermissionDenied(format!(
        ...
        if code == 429 {
            ...
            if attempt >= MAX_RETRY_ATTEMPTS {
                ...
                return Err(WebhookError::Unavailable(format!(
        ...
        if status.is_server_error() {
            return Err(WebhookError::Unavailable(format!("HTTP {code}: server error")));
        }
        // Other 4xx
        return Err(WebhookError::Unavailable(format!(
...
mod tests {
    ...
    async fn discord_post_5xx_returns_webhook_unavailable() {
        ...
        let result = post(&url, &payload).await;
        assert!(
            matches!(result, Err(WebhookError::Unavailable(_))),
            "5xx must return WebhookUnavailable: {result:?}"
        );
        assert_webhook_error(result, "webhook unavailable: HTTP 503: server error");
    }
    ...
    async fn discord_post_401_returns_permission_denied() {
        ...
        let result = post(&url, &payload).await;
        assert!(
            matches!(result, Err(WebhookError::PermissionDenied(_))),
    ...
    /// E4 characterization: the exact rendered bytes of a failed `post`,
    /// pinned before the crate split moved this module. The exit code each
    /// variant maps to is pinned once, by the `From<WebhookError> for
    /// MirrorError` identity table in `ocx_mirror_error`'s tests.
    fn assert_webhook_error(result: Result<(), WebhookError>, expected: &str) {
        let error = result.expect_err("the POST must fail");
        assert_eq!(error.to_string(), expected);
    }

    // `post`'s client-build failure arm is not pinned: `ocx_mirror_http::builder`
    ...
    async fn discord_post_unexpected_4xx_returns_webhook_unavailable() {
        ...
        assert_webhook_error(result, "webhook unavailable: HTTP 404: unexpected client error");
    }
    ...
    async fn discord_post_network_error_withholds_the_webhook_url() {
        ...
        assert_webhook_error(result, "webhook unavailable: network error: error sending request");
    }
    ...
    async fn discord_post_timeout_returns_webhook_unavailable() {
        ...
        assert_webhook_error(result, "webhook unavailable: request timed out: error sending request");
    }
    ...
    async fn discord_post_429_shared_scope_in_error() {
        ...
        let result = post(&url, &empty_payload()).await;
        match &result {
            Err(WebhookError::Unavailable(msg)) => {
                assert!(
                    msg.contains("shared"),
