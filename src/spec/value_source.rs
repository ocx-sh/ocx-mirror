...
//! `source.url_index` already uses ([`crate::spec::source`]), so an operator
...
/// Bound on one value fetch. [`crate::http::builder`] sets only a connect
...
impl ValueSource {
    ...
    pub(crate) async fn resolve(&self, field: &str, spec_dir: &Path) -> Result<String, MirrorError> {
        let bytes = match self {
            ...
            Self::Generator(generator) => crate::source::generator::run(generator, spec_dir)
                .await
...
async fn fetch(url: &str) -> Result<Vec<u8>, MirrorError> {
    ...
    if let Some(credential) = crate::auth::resolve(&parsed)? {
    ...
    crate::http::read_capped(&mut response, url, VALUE_FETCH_CEILING).await
}
