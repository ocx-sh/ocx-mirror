...
/// carry no `variant` segment (validated), so this is the canonical string with
/// any `+libc.*` suffix stripped.
    ...
    fn rejects_any_key_and_variant_segments() {
        ...
        assert!(
            validate("linux/arm64/v8: ~\n")
                .iter()
                .any(|e| e.contains("variant segment is not supported"))
        );
    }

    #[test]
    fn rejects_dual_libc_and_unknown_features() {
        // One `+`, comma-separated features — the v0.5.3 key grammar. Two
        // libcs parse fine and are rejected here, not by `FromStr`.
        assert!(
