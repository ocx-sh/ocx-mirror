...
- Commands: `command/{sync,check,validate,target_registry,options}.rs`; `command/pipeline/{plan,prepare,push,notify,mod}.rs` + `generate/` (workflow renderer, templates baked in).
...
- Generated workflows are a drift-guarded surface (`pipeline generate ci --check` exits 65). Edit templates in `command/pipeline/generate/templates/`, not output.
