---
name: commit
description: Use when the user says "commit", "/commit", "save progress", or asks to land working changes on a feature branch. ocx-mirror working-phase posture — minimises commit count, can amend rolling Checkpoints, offers a one-time PR prompt. For pre-merge cleanup, use `/finalize`.
user-invocable: true
...
Commit working tree during active dev on feature branch. **Working phase** half of two-phase branch workflow — rebasing-phase cleanup for `main` lives in `/finalize`.
...
Optional free-text arg. Treat as intent hint (e.g., `fix cascade tag ordering bug`), not full message — still draft final message. Flags:
...
On feature branch, prefer one of:
...
- **Never** `Co-Authored-By` (repo convention)
...
1. **Create feature branch + PR** — derive name from commit subject (e.g. `feat/plan-resolved-assets`), branch from HEAD, commit there, `gh pr create` with title from subject and body summary
...
- **Verification is on you — no pre-commit hooks here.** For code changes, run at minimum `task` (fast check) before committing; run `task verify` before declaring the work done or handing to `/finalize`. AI-config/docs-only changes may commit on a fast check.
- Commit with HEREDOC:
