...
| `filter.rs` | `filter_versions()`: apply bounds, prerelease skip, backfill cap. Also `pep440_sort_key()` — the total order over version strings that `plan` and `push` sort by — and `version_cmp()`/`within_bounds()`, the min-inclusive/max-exclusive comparator every bound routes through (`versions.min`/`max`, `select_pypi_candidates`, per-platform windows, `exclude:` ranges): `ocx_package::version::Version` first, PEP 440 for the ≥4-component releases it rejects, fail-open when neither parses |
