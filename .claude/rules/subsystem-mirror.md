...
| `spec/assets.rs` | `AssetPatterns` (platform → regex[] mapping). Keys are `os/arch[/variant][+libc.<flavor>[,…]]` parsed via `ocx_oci::Platform::from_str`; a `+libc.glibc`/`+libc.musl` suffix lands in `os_features` and publishes as an OCI `os.features` entry |
...
| `spec/tests_config.rs` | `TestEntry` (`name` plus exactly one of `command`/`script`/`script_inline`) and `TestKind`; top-level `tests:` schema. `Serialize` as well as `Deserialize`: this type **is** `plan.json`'s `legs.<key>.tests[]` shape, so a field added here widens the plan's wire contract |
| `spec/platforms_config.rs` | `PlatformConfig`, `ContainerConfig` (`image`/`shell`/`id`/`setup` — `setup` provisions the leg's image once per leg via `docker build`); `platforms:` matrix schema; per-platform version applicability (`min_version`/`max_version`/`exclude` of `ExcludeEntry`+`Severity`). `runner` is a forge-neutral **label set** — still required, `Vec<String>` through a hand-rolled visitor taking one label or a list (GitHub reads it as `runs-on`, GitLab as `tags`), and the renderer re-emits the single-label spelling unchanged. `ContainerConfig::resolved_id`/`resolved_shell` and `PlatformConfig::native_shell` sit on the spec types because the CI renderer *and* `plan/legs.rs` both derive from them; a second copy hands one of them a join key the other never wrote |
...
| `annotations.rs` | OCI annotations on every published index: `ocx package push --ci-annotations=<provider>` (GitHub + GitLab: source, revision, created, version) under the spec's `annotations:` overlay, rendered as the `--annotation KEY=VALUE` tail that overrides it. The provider is named, never autodetected — a bare `--ci-annotations` outside CI is exit 64. `image.created` is the one key the mirror pins itself, once per run and only on GitHub Actions, which exposes no pipeline clock: without it each platform's own `ocx package push` would date one version's legs apart |
...
| `filter.rs` | `filter_versions()`: apply bounds, prerelease skip, backfill cap — it takes the run's `ResolvedBounds` rather than re-reading the spec's strings, so a bound fetched from a vendor pointer reaches the same seam a literal one does. Also `pep440_sort_key()` — the total order over version strings that `plan` and `push` sort by — and `version_cmp()`/`within_bounds_ex()`, the comparator every bound routes through: `ocx_package::version::Version` first, PEP 440 for the ≥4-component releases it rejects, fail-open when neither parses. `within_bounds()` is the 3-arg min-inclusive/max-exclusive wrapper the per-platform windows and `exclude:` ranges keep; `versions.min`/`max` carry their own edges and go through `ResolvedBounds::admits`, as does `select_pypi_candidates` |
...
- `pylock` / `pypi`: env sources — see `spec/wheels.rs` and the `python:` block
...
| `SpecUsageError` | 64 (UsageError) | Invalid `mirror.yml` usage: hardcoded webhook URL, empty `tests:`, ambiguous shell, `ocx_install:` block, a malformed `sign:` block (`spec/sign_config.rs`), a credential-named key anywhere in the document (`spec/prescan.rs` deny-list), or a malformed `OCX_MIRROR_URL_REWRITE` — no `=` separator, an empty half, a non-`http(s)` prefix, or userinfo in `to`; the message names the variable and never its value, because it reaches a durable CI log |
