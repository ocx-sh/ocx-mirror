...
Path-scoped rule (auto-loads on source-work surfaces: `src/**`, `crates/**`, `tests/**`, `test/**`, `.claude/**`, `Cargo.toml`, `Cargo.lock`). Referenced from [workflow-intent.md](./workflow-intent.md) when work classified as bug fix. Enforce root-cause discipline: understand bug before fix.
...
- [ ] All existing tests still pass (`task rust:verify` for Rust changes)
...
**Gate**: `task rust:verify` passes. Regression test passes. Manual verify confirms fix.
...
Bug-fix plans created from the template carry a `## Status` block at the top — same schema and mutation protocol as feature plans. See [`meta-plan-status.md`](./meta-plan-status.md).
