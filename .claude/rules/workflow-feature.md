...
8. **Implement** — `worker-builder` (focus: `implementation`) fills stub bodies until all tests pass. Gate: `task rust:verify` succeeds (see Quality Gate section in `subsystem-mirror.md`).
9. **Review-Fix Loop** — Apply canonical Review-Fix Loop to feature diff. See [`workflow-swarm.md`](./workflow-swarm.md#review-fix-loop). Run **`task rust:verify`** during the loop, NOT full `task verify`.
10. **Cross-Model Adversarial Pass** — Documented inside canonical Review-Fix Loop (see [`workflow-swarm.md`](./workflow-swarm.md#review-fix-loop)). Opt-out flag: `--no-codex` on `/swarm-execute`.
...
Every plan in `.claude/state/plans/plan_*.md` carries a `## Status` block at the top (after H1, before first content section). Block fields: `Plan`, `Active phase`, `Step`, `Last update`. Read+mutated by `/swarm-plan` (init), `/swarm-execute` (phase entry/advance), `/swarm-review` (round/verdict), `/commit` (Last update), `/finalize` (refuse if active, mark `finalized` on done). Global pointer `.claude/state/current_plan.md` (gitignored) names the active plan. Schema + mutation table → [`meta-plan-status.md`](./meta-plan-status.md).

## Quality Gates

Run `task verify` (fmt check + clippy + license + build + unit tests + acceptance tests). See `.claude/rules/quality-core.md` for canonical gate list.
