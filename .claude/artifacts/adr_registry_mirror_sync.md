...
package's root, fetch each referenced manifest **by digest**, HEAD every descriptor digest at the
destination. The estimate is then the sum of the `size` field of every descriptor whose HEAD missed,
counting each distinct digest once. That is the exact byte count a real run would transfer, not a
guess — descriptor `size` is authoritative in an OCI manifest. Cost: the same request count as a real
run minus every blob GET and every push, which is the point.
